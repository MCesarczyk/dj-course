<template>
    <div class="card p-6 mb-6 animate-fade-in">
            ...
            >
                <div class="relative w-24 h-24">
                    ...
                    >
                        ...
                        >
                        <span
                            class="text-xs text-gray-500 dark:text-gray-400 leading-none mt-0.5"
                            >{{ ring.unit }}</span
                        >
                    </div>
                </div>
                <span
                    class="text-xs font-semibold text-gray-600 dark:text-gray-300 text-center"
                    >{{ ring.label }}</span
                >
                <span class="text-xs font-bold" :style="{ color: ring.color }"
                    >{{ ring.progress }}%</span
                >
            </div>
        </div>
        ...
        <!-- Footer link -->
        <div
            class="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700 flex items-center gap-1.5 group cursor-pointer"
        >
            <span
                class="text-sm text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors"
            >
                View Activity Details
            </span>
            <ArrowTopRightOnSquareIcon
                class="w-3.5 h-3.5 text-gray-400 group-hover:text-gray-500 dark:group-hover:text-gray-300 transition-colors"
            />
        </div>
    </div>
</template>
...
// ── Default data derived from mock requests ──────────────────────────────────

function defaultRings(): StatRing[] {
    ...
    const activeWarehousing = mockWarehousingRequests.filter((r) =>
        ["APPROVED", "PENDING_ARRIVAL", "RECEIVED", "STORED"].includes(
            r.status,
        ),
    ).length;
...
function defaultStatusItems(): StatusItem[] {
    ...
    const pendingReviewCount =
        mockTransportationRequests.filter((r) => r.status === "SUBMITTED")
            .length +
        mockWarehousingRequests.filter(
    ...
    return [
        ...
        },
        {
            ...
            count: deliveredTransport + completedWarehousing,
            done: true,
            color: "#22c55e",
        },
        {
...
// ── Effective values (prop override or derived default) ───────────────────────

const rings = computed<StatRing[]>(() => props.rings ?? defaultRings());
const statusItems = computed<StatusItem[]>(
    () => props.statusItems ?? defaultStatusItems(),
);
</script>
